...
export const configProxyEventHandlers = (proxy: Server, target: string, options: ConfigProxyEventHandlersOptions) => {
  proxy.on('error', async (err, req: any, res) => {
    ...
    if (req.frontegg.retryCount >= maxRetries && res instanceof ServerResponse) {
