...
export async function validatePermissions(req: Request, res: Response, context: FronteggContextResolverRes) {
  ...
  console.log(`No matching permission for ${req.method} ${url}. Permissions - ${allowedOperations}`);
...
export function enableCors(req: Request, res: IncomingMessage) {
  ...
  if (req.headers.origin) {
    console.log(`enableCors - going to set access-control-allow-origin to ${req.headers.origin}`);
    res.headers['access-control-allow-origin'] = req.headers.origin;
    res.headers['access-control-allow-credentials'] = 'true';
  }
}

export async function callMiddleware(req: Request, res: Response, middleware: AuthMiddleware): Promise<void> {
